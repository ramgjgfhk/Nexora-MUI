...
import {
  Button, Popover, Box, IconButton, Select, MenuItem,
  Typography, FormControl, Paper
} from '@mui/material';
...
export default function AdvancedSortButton({ sortModel, setSortModel, sortableColumns }) {
  ...
  const handleAddRule = () => {
    const unused = sortableColumns.find(c => !sortModel.some(r => r.colId === c.colId));
    if (unused) {
  ...
  return (
    <>
      ...
      <Popover
        ...
        PaperProps={{ sx: { p: 2, width: 350 } }}
      >
        <Typography variant="subtitle1" mb={1}>Advanced Sorting</Typography>
